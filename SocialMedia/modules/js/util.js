		...
		if(params.launchmode == 1){
			...
			if(params.launchparams != null){
				...
				if('name' in ProfileData){ 
					segData.push({"lblKey" : "Name","lblValue":ProfileData["name"]});
				...
				if('screen_name' in ProfileData) {
					segData.push({"lblKey" : "Screen_name","lblValue":ProfileData["screen_name"]});
					frmLoginSuccess.lblUsername.text = "Welcome "+	ProfileData["screen_name"];
				}
				if('email' in ProfileData) segData.push({"lblKey" : "Email","lblValue":ProfileData["email"]});
				if('gender' in ProfileData) segData.push({"lblKey" : "Gender","lblValue":ProfileData["gender"]});
				if('birthday' in ProfileData) segData.push({"lblKey" : "Birthdate","lblValue":ProfileData["birthday"]});
				if('family_name' in ProfileData) segData.push({"lblKey" : "family_name","lblValue":ProfileData["family_name"]}); 
				if('given_name' in ProfileData) segData.push({"lblKey" : "given_name","lblValue":ProfileData["given_name"]});
				if('verified_email' in ProfileData) segData.push({"lblKey" : "verified_email","lblValue":ProfileData["verified_email"]});
				...
				if('username' in ProfileData) segData.push({"lblKey" : "Username","lblValue":ProfileData["username"]});		
				...
				if('hometown' in ProfileData) segData.push({"lblKey" : "Hometown","lblValue":ProfileData["hometown"]["name"]}); 
				//alert(ProfileData["work"][0]["location"]["name"]);
				
				if(ProfileData["work"][0]["location"]["name"] != null) 
					segData.push({"lblKey" : "Location","lblValue":ProfileData["work"][0]["location"]["name"]});
					
			
				if('bio' in ProfileData) segData.push({"lblKey" : "Bio.","lblValue":ProfileData["bio"]});
				if('work' in ProfileData) segData.push({"lblKey" : "Work","lblValue":ProfileData["work"][0]["employer"]["name"]});
				if('description' in ProfileData) segData.push({"lblKey" : "Bio.","lblValue":ProfileData["description"]});
 	...
 	if(method=="POST")
 	{
 		var postbody = new kony.net.FormData();  
      	var jsonObj = eval('(' + postdata + ')');
   		for(var key in jsonObj) { postbody.append(key,jsonObj[key]);}
        kony.print("postdata::"+JSON.stringify(postbody));
 	 	request.send(postbody);
 	 } else
 		request.send();			//Available on all platforms except BlackBerry 10, Windows Platform and SPA.
...
function chkIfPropertiesConfigured(clientName){
	...
	if(clientName == "FB"){
		if(FBConfig.clientID!="" && FBConfig.clientID!=undefined && FBConfig.clientID!=null && FBConfig.clientID.search("<")==-1 &&
		 FBConfig.clientSecret!=null && FBConfig.clientSecret!=undefined && FBConfig.clientSecret!="" && FBConfig.clientSecret.search("<")==-1 )
			return true;
		else
			return false;
	}else if(clientName == "Google"){
		if(GoogleConfig.clientID!="" && GoogleConfig.clientID!=undefined && GoogleConfig.clientID!=null && GoogleConfig.clientID.search("<")==-1 && 
		   GoogleConfig.clientSecret!="" && GoogleConfig.clientSecret!=undefined && GoogleConfig.clientSecret!=null && GoogleConfig.clientSecret.search("<")==-1)
			return true;
		else
			return false;
	}else {
		if(LinkedInConfig.clientID!="" && LinkedInConfig.clientID!=undefined && LinkedInConfig.clientID!=null && LinkedInConfig.clientID.search("<")==-1 && 
		   LinkedInConfig.clientSecret!="" && LinkedInConfig.clientSecret!=undefined && LinkedInConfig.clientSecret!=null && LinkedInConfig.clientSecret.search("<")==-1)
			return true;
		else
			return false;
