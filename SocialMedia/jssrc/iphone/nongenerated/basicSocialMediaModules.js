...
function loginAccountAuth(eventObj) {
    ...
    if (eventObj.id == "btnFacebook") {
        ...
        if (chkIfPropertiesConfigured("FB")) {
            ...
            frmProfileBrowser.brwsrId.requestURLConfig = {
                "URL": fbURL,
...
function showAppData(response) {
    ...
    if (response["name"] != null && response["name"] != undefined) {
        ...
        segData.push({
            "lblKey": "Name",
            "lblValue": response["firstName"] + " " + response["lastName"]
        });
        frmShowOptions.lblUsername.text = "Name: " + response["firstName"] + " " + response["lastName"];
    }
    if ('email' in response) segData.push({
        "lblKey": "Email",
        "lblValue": response["email"]
    });
    if ('gender' in response) segData.push({
        "lblKey": "Gender",
        "lblValue": response["gender"]
    });
    if ('family_name' in response) segData.push({
        "lblKey": "family_name",
        "lblValue": response["family_name"]
    });
    if ('given_name' in response) segData.push({
        "lblKey": "given_name",
        "lblValue": response["given_name"]
    });
    if ('headline' in response) {
        frmShowOptions.lblHeadLine.text = response["headline"];
        segData.push({
            "lblKey": "HeadLine",
            "lblValue": response["headline"]
        });
